...
const removeCartProduct = async (li, id) => {
  ...
  const valueTotal = Number(subTotal.innerText);
  const total = valueTotal - productElement.price;
...
const addItemCart = async (id) => {
  ...
  const newProduct = createCartProductElement({
    ...
    pictures: productElement.pictures,
  });
  cardProducts.appendChild(newProduct);
  const savedCards = getSavedCartIDs();
  savedCards.push(id);
  localStorage.setItem('cartProducts', JSON.stringify(savedCards));
...
export const createProductElement = ({ id, title, thumbnail, price }) => {
  ...
  cartButton.addEventListener('click', () => addItemCart(id));
